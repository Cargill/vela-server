...
func testLimits(t *testing.T, db Interface, resources *Resources) {
	...
	// delete the org build limits
	for _, limit := range resources.Limits {
		err := db.DeleteOrgBuildLimit(ctx, limit.GetOrg())
		if err != nil {
			t.Errorf("unable to delete org build limit for %s: %v", limit.GetOrg(), err)
		}
	}

	methods["DeleteOrgBuildLimit"] = true
